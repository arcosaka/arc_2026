...
void ADCInit();
void UARTInit(uint32_t baudrate, BYTE parity, BYTE stop);
...
volatile uint32_t UARTWriteCount = 0;
volatile uint32_t UARTReadCount = 0;
volatile uint32_t UARTBuffCount = 0;
...
**
*****************************************************************************/
void UARTInit(uint32_t baudrate, BYTE parity, BYTE stop)//,uint8_t ,uint8_t )
{
    uint32_t Fdiv;
    uint32_t regVal;
   ...
   // LPC_IOCON->PIO1_7 &= ~0x07;
    ...
    if (parity == 0 || parity == 1)
        lcrtemp |= (parity << 4) | (1 << 3);

    if (stop == 2)
        lcrtemp |= 1 << 3;

    LPC_UART->LCR = 0x83 | lcrtemp;             /* 8 bits, no Parity, 1 Stop bit */
...
uint32_t SciRx(uint8_t* buffer, uint32_t size) {
    ...
    while (
     ...
     && (UARTBuffCount > 0)
        ...
        UARTBuffCount--;
        u1_byte = UARTBuffer[UARTReadCount];
        UARTReadCount++;
        UARTReadCount %= BUFSIZE;
        buffer[u4_retsize++] = u1_byte;
    }
    return u4_retsize;
}
